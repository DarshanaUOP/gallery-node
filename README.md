...
- **HEVC/H.265 on-the-fly transcode** — `/api/video/<id>` detects HEVC sources (common in iPhone recordings) via each item's codec metadata and transcodes them server-side to H.264/AAC on the fly, piping ffmpeg's output straight into the response as it's generated — nothing is ever written to disk, so no converted copy is kept in storage. Fixes playback in Firefox, which has no HEVC decoder, and in browsers with inconsistent hardware-decoder support. Trade-off: since the stream's length isn't known ahead of time, seeking/scrubbing isn't available on a video that needed this fallback. Falls back to serving the original file if ffmpeg is unavailable
...
- `.MOV`/`.MP4` files recorded on iPhone with HEVC (H.265) codec — Firefox ships no HEVC decoder at all, and Chrome's support depends on OS/hardware decoding, so it's inconsistent there too. As of the current `app.py`, `/api/video/<id>` detects HEVC sources (from the codec already recorded in each item's metadata, or a quick on-demand ffprobe check for items indexed before that field existed) and transcodes them to H.264/AAC on the fly, streaming ffmpeg's output directly into the response — nothing is written to disk, so there's no converted copy left in storage
- **Seeking/scrubbing isn't available on a video that needed this fallback** — since the transcode is generated as it streams, its total length isn't known up front, so standard HTTP byte-range seeking doesn't apply. Playback runs start-to-finish; dragging the progress bar won't work correctly. This is a deliberate trade-off for not persisting converted files — if you'd rather have seeking work (at the cost of a cached converted copy on disk, regenerated only when the source changes), let me know and I can switch back to that approach
- If ffmpeg isn't installed, the HEVC transcode is skipped and the original file is served as before — same "browser can't play this format" fallback message you'd have seen previously
