...
```
luminary/
├── Luminary.spec                 # PyInstaller spec file — auto-generated by the build scripts
├── README.md
...
`Luminary.spec` and the top-level `build/` folder are PyInstaller's own working files, regenerated on every run of the build scripts — see `BUILD.md` and `scripts/Readme.md` for packaging a standalone executable.
